...
\begin{document}
	...
	A non-favorable setup for the knockoff is the following:
	The model is a well-specified liner model; the number of alternatives is not large; correlations between $X_j$ are considerable. 
	In this setup, the knockoff framework may be unboundedly worse that the simplest regression with FWER control.
	Why? 
	...
	The ratio between the cutoff of some FDR controlling procedure, and FWER control with Bonferroni, is thus $\bigl\{1+\log(|S|)/\log (p/q)\bigr\}^{1/2}$, so that if $|\mathcal{S}|\ll p$, and the methods have similiar power for identical hypotheses. 
	...
	The predictors are correlated in pairs: $X\in R^{2p}$, $(X_1,X_2),\dots,(X_{2p-1},X_{2p})$ are i.i.d.,   $(X_{2j-1},X_{2j})^T\sim \mathcal{N}_2(0,\Sigma)$ where $\Sigma=\begin{pmatrix}1 & \rho\\\rho &1\end{pmatrix}$.
	\citet{CandesPanninggoldmodelX2018} show that the knockoffs are given by
	\begin{equation*}
	(\tilde X_{2j-1},\tilde X_{2j}|X=x) \sim 
	\mathcal{N}_2\Bigl((I+\lambda \Sigma^{-1})(x_{2j-1},x_{2j})^T,\hspace{1em} 2\lambda I-\lambda^2\Sigma^{-1}\Bigr),
	\end{equation*}
	where $\lambda<2(1-\rho)$.
	Our calculations show the effect size needed to be detected with knockoffs with reasonable power is larger by at least a factor of $(1-\rho^2)^{-1/2}$ compared to classical linear regression with Bonferonni FWER control (for the optimal selected $\lambda$ which is the minimal). 
	If the correlations are strong, i.e. $\rho \approx 1$, the power loss is considerable.
	Combining with the fact that in our setup the FDR and FWER rejection cutoffs are similar, we conclude this is clearly an unfavorable setup for knockoffs. 
